...
# Ensure the host git global config contains safe.directory entries for both
# the host path and the container path for the zephyr workspace. This writes
# to the host ~/.gitconfig so it's visible inside the container via the mount
# added to $COMMAND above.
ensure_host_gitconfig_safe() {
  local host_zephyr_path
  host_zephyr_path="$(pwd)/zephyr"

  # Ensure the gitconfig file exists
  mkdir -p "$(dirname "$HOME/.gitconfig")"
  touch "$HOME/.gitconfig"

  # Add both the container-visible path and the host path as safe directories.
  git config --global --add safe.directory /zmk_bhv/zephyr || true
  git config --global --add safe.directory "$host_zephyr_path" || true
}
...
build_target() {
  ...
  while IFS='|' read -r board shield snippet cmake_args artifact_name; do
    if [ "$artifact_name" = "$target_name" ]; then
      ...
      cmake_args_array+=("-DZMK_CONFIG=/zmk_bhv/config")
      if [ -n "$shield" ]; then
...
check_init() {
  ...
  if [ ! -d "./zmk" ]; then
    log_warning "ZMK firmware not found (zmk_bhv/ not found)"
    needs_init=1
  fi
